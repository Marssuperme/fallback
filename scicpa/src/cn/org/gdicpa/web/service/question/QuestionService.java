package cn.org.gdicpa.web.service.question;
...
import cn.org.gdicpa.web.service.question.model.TreeNode;
...
public class QuestionService {
	...
	public final static String TYPE_EAUDIT = "eaudit";
	
	public final static String TYPE_PUBLICSERVICE = "publicService";
	...
	public List getTree(String id, String type, String mine) {
		...
		try {
			if (TYPE_POLICY.equals(type)) {
				// 展开法律法规树
				String sql = "select distinct a.* from p_policytype a,( \n"
						+ " 	select a.*,b.userId from ( \n"
						+ " 		select a.id as policyId,b.*  from p_policy a,(  \n"
						+ "  		select b.* from p_policytype a "
						+ "   		inner join  \n"
						+ "   		p_policytype b on b.fullPath like a.fullPath+'%' \n"
						+ "			where a.parentid = '"
						+ id
						+ "' "
						+ "	 	) b where a.typeid= b .id  \n"
						+ "	) a,p_question b where a.policyId = b.typeid  \n"
				...
				// 展开案例库树
				String sql = "select distinct a.* from p_casesType a,( \n"
						+ " 	select a.*,b.userId from ( \n"
						+ " 		select a.id as caseId,b.*  from p_cases a,(  \n"
						+ "  		select b.* from p_casesType a "
						+ "   		inner join  \n"
						+ "   		p_casesType b on b.fullPath like a.fullPath+'%' \n"
						+ "			where a.parentid = '"
						+ id
						+ "' "
						+ "	 	) b where a.typeid= b.id  \n"
						+ "	) a,p_question b where a.caseId = b.typeid  \n"
						+ ") b where b.fullPath like a.fullPath+'%' and a.parentid = '"
						+ id + "'   \n";
				if (!"".equals(mine)) {
					sql += "and userId='" + mine + "'";
			...
			}else if (TYPE_EAUDIT.equalsIgnoreCase(type)) {// E 审通

			}else if (TYPE_PUBLICSERVICE.equalsIgnoreCase(type)) {// 公共服务平台
				...
				TreeNode eAudit = new TreeNode();
				eAudit.setId("03");
				eAudit.setText("E审通审计软件优化");
				eAudit.setValue("eaudit");
				eAudit.setShowcheck(false);
				eAudit.setHasChildren(true);
				treeList.add(eAudit);
				
				TreeNode publicService = new TreeNode();
				publicService.setId("04");
				publicService.setText("公共服务平台优化");
				publicService.setValue("publicService");
				publicService.setShowcheck(false);
				publicService.setHasChildren(true);
				treeList.add(publicService);
				
				
				TreeNode otherTree = new TreeNode();
				otherTree.setId("999999");
	...
	public List<Map<String,String>> getTypeList(String ctype) throws Exception {
		...
		if (TYPE_POLICY.equals(ctype)) {
			tableName = "p_policyType";
		} else if(TYPE_CASES.equals(ctype)) {
			tableName = "p_casesType";
		} 
	...
	public boolean isAnswer(String questionId, String loginId) throws Exception {
		...
		String sql = " select id from p_answer where questionId=? and userId=? ";
	...
	public Question getQuestion(String id) {
		...
		ResultSet rs = null;
		try {
			String sql = "select id,a.ctype,typeId,title,question,a.userId,createDate,a.state,fullPath,rewardMark,b.loginname,explan,explanDate "
					+ " from p_question a left join k_user b on a.userId=b.loginid  where id=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, id);
			rs = ps.executeQuery();
	...
	public String getPolicyTypeIds(String fullPath) {
		...
		ResultSet rs = null;
		try {
			String sql = "select distinct a.id from p_policy a,("
					+ "	select b.* from p_policytype a " + "	inner join "
					+ "	p_policytype b on b.fullPath like a.fullPath+'%' "
	...
	public String getCaseTypeIds(String fullPath) {
		...
		try {
			String sql = "select distinct a.id from p_cases a,("
					+ "	select b.* from p_casestype a " + "	inner join "
					+ "	p_casestype b on b.fullPath like a.fullPath+'%' "
					+ "	where b.fullPath = '" + fullPath + "'"
					+ ") b where a.typeid= b.id ";
...
//			String sql = "insert into p_question(id,ctype,typeId,title,question,userId,createDate,state,fullPath,rewardMark) values(?,?,?,?, ?,?,?,? ,?,?) select scope_identity()";
			String sql = "insert into p_question(id,ctype,typeId,title,question,userId,createDate,state,fullPath,rewardMark) values(?,?,?,?, ?,?,?,? ,?,?) ";
			ps = conn.prepareStatement(sql);
			int i = 1;
			ps.setString(i++, id);
...
//			}
	...
	public void add(Answer answer) {
		...
		try {
			...
			String sql = "insert into p_answer(id,questionId,answer,userId,answerDate,isBest) values(?,?,?,?,?, ?)";
			ps = conn.prepareStatement(sql);
...
//			}
	...
	public List<Question> getQuestions(String type, String typeId) {
		...
		try {
			String sql = "select top 5 id,title,question,userId,createDate,state,fullPath from p_question where ctype=? and typeId=? order by createDate desc,id desc";
			ps = conn.prepareStatement(sql);
			ps.setString(1, type);
	...
	public List<Answer> getAnswers(String questionId) {
		...
		try {
			String sql = "select id,questionId,answer,a.userId,answerDate,isBest,b.loginname from p_answer a"
					+ " left join k_user b on a.userId = b.loginid "
					+ " where questionId=? and (isBest<>'1' or isBest is null) order by answerDate desc,id desc";
			ps = conn.prepareStatement(sql);
	...
	public Answer getAnswer(String answerId) {
		...
		ResultSet rs = null;
		try {
			String sql = "select id,questionId,answer,userId,answerDate,isBest from p_answer where id=? ";
			ps = conn.prepareStatement(sql);
	...
	public Answer getBestAnswer(String questionId) {
		...
		ResultSet rs = null;
		try {
			String sql = "select a.id,questionId,answer,a.userId,answerDate,isBest,b.loginName from p_answer a"
					+ " left join k_user b on a.userId = b.loginid "
					+ " where questionId=? and isBest='1' "
					+ " order by answerDate desc,id desc";
			ps = conn.prepareStatement(sql);
			ps.setString(1, questionId);
			rs = ps.executeQuery();
			Answer answer = new Answer();
	...
	public int updateAnswer(Answer answer) {
		...
		try {
			String sql = "update p_answer set answer=?,userId=?,answerDate=?,isBest=? where id=? ";
			ps = conn.prepareStatement(sql);
	...
	public void updateFullPath(String questionId, String fullPath) {
		...
		PreparedStatement ps = null;
		try {
			String sql = "update p_question set fullPath=? where id=?";
			ps = conn.prepareStatement(sql);
