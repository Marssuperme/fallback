package cn.org.gdicpa.web.action.bbkjdshb;
...
public class KjdshbAction extends MultiActionController{
	private static String LIST = "/bbkjdshb/list.jsp";
	private static String ADDANDEDIT = "/bbkjdshb/addAndEdit.jsp";
	private static String SEARCH = "/bbkjdshb/search.jsp";
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteKjdshbTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbkjdshb.do?method=index&typeid="+typeid);
		return null;
	}
