...
public class InternshipAction extends MultiActionController {

	private final String _VIEW = "/internship/list.jsp";
	private final String _LOOK_VIEW = "/internship/view.jsp";
	private final String _ADD_VIEW = "/internship/add.jsp";
	private final String _DEL_VIEW = "/internship/del.jsp";
...
//			pp.setColumnWidth("150,50,20,10,10,50,10");
	...
	public ModelAndView save(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if("".equals(result)){
				response.sendRedirect(request.getContextPath()+ "/common/internship.do");
			}else{
				PrintWriter out = null ;
				response.setContentType("text/html;charset=UTF-8") ;
				out = response.getWriter() ;
				out.println("<script>alert(\""+result+"\");window.location=\""+request.getContextPath()+ "/common/internship.do?method=add\"</script>");
				out.close();
			}
	...
	public ModelAndView saveport(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if("".equals(result)){
				response.sendRedirect(request.getContextPath()+ "/common/internship.do?method=view&topicid="+topicid+"&random="+DELUnid.getNumUnid() );
			}else{
				PrintWriter out = null ;
				response.setContentType("text/html;charset=UTF-8") ;
				out = response.getWriter() ;
				out.println("<script>alert(\""+result+"\");window.location=\""+request.getContextPath()+ "/common/internship.do?method=view&topicid="+topicid+"&random="+DELUnid.getNumUnid()+"\"</script>");
				out.close();
			}
	...
	public ModelAndView del(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			response.setContentType("text/html;charset=UTF-8") ;
			out = response.getWriter() ;
			out.println("<script>alert(\"删除数据成功\");window.location=\""+request.getContextPath()+ "/common/internship.do?method=indexdel&loginid="+loginid+"\"</script>");
			out.close();
