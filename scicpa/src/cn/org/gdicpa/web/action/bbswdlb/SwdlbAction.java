package cn.org.gdicpa.web.action.bbswdlb;
...
public class SwdlbAction extends MultiActionController{
	private static String LIST = "/bbswdlb/list.jsp";
	private static String ADDANDEDIT = "/bbswdlb/addAndEdit.jsp";
	private static String SEARCH = "/bbswdlb/search.jsp";
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteSwdlbTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbswdlb.do?method=index&typeid="+typeid);
		return null;
	}
