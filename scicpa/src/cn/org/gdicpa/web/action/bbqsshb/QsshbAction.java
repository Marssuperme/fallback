package cn.org.gdicpa.web.action.bbqsshb;
...
public class QsshbAction extends MultiActionController{
	private static String LIST = "/bbqsshb/list.jsp";
	private static String ADDANDEDIT = "/bbqsshb/addAndEdit.jsp";
	private static String SEARCH = "/bbqsshb/search.jsp";
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteQsshbTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbqsshb.do?method=index&typeid="+typeid);
		return null;
	}
	...
	public ModelAndView addAndEdit(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			if(param.equals("search")){
				...
				// 清算审计
				QsshbService qs = new QsshbService(conn);
				String p = request.getParameter("p");   //  查看操作还是还是作废操作的判断依据
				String GUID = request.getParameter("GUID");
				// 代报备内容
