package cn.org.gdicpa.web.action.bbzcpgb;
...
public class ZcpgbAction extends MultiActionController{
	private static String LIST = "/bbzcpgb/list.jsp";
	private static String ADDANDEDIT = "/bbzcpgb/addAndEdit.jsp";
	private static String SEARCH = "/bbzcpgb/search.jsp";
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteZcpgbTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbzcpgb.do?method=index&typeid="+typeid);
		return null;
	}
