package cn.org.gdicpa.web.action.bbqchzb;
...
public class QchzbAction extends MultiActionController{
	private static String LIST = "/bbqchzb/list.jsp";
	private static String ADDANDEDIT = "/bbqchzb/addAndEdit.jsp";
	private static String SEARCH = "/bbqchzb/search.jsp";
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteQchzbTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbqchzb.do?method=index&typeid="+typeid);
		return null;
	}
