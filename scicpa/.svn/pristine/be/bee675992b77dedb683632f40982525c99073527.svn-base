...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="data_tb" align="center">
 	  <tr>
	    <td width="160" height="53" class="data_tb_alignright">其它承办的项目<font color="#FF0000">&nbsp;[*]</font></td>
	    <td width="577" height="53" colspan="3" align="left" >
	    	<textarea title="其它承办的项目" id="qtcbdxm" name="qtcbdxm" style="width: 450px;height: 50px;" class="required">${bt.qtcbdxm }</textarea>
	    </td>
	  </tr>
</table>
...
function saveCompanyJudge(){
	var bbqtbguid = document.getElementById("bbqtbguid").value;
	if(bbqtbguid=="" || bbqtbguid==null){
		document.thisForm.action="${pageContext.request.contextPath}/common/bbbbqtb.do?method=addBbqtbTable";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}else{
		document.thisForm.action="${pageContext.request.contextPath}/common/bbbbqtb.do?method=updateBbqtbTableById";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}
...
// 返回
function goback(){	
	document.thisForm.action="${pageContext.request.contextPath}/common/bbbbqtb.do?method=index&typeid=bbqt001";
	thisForm.submit();
}
