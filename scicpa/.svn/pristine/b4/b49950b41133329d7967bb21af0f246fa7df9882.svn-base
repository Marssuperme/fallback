...
<A class="tab curtab" id=cpatab href="javascript:setTab('search','cpa')">CPA任职资格预查</A>
...
<div class=tabcontent id="cpa" style="text-align: center; width: 100%">
	<div style="background-color: #e8f7fc ! important;width: 100%;">
		...
		<c:if test="${iyear==now}">
	   	...
	   	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input icon='icon-query' type='button' onclick="f_query();" value='查看年检规则'>
    </div>
	<!-- CPA信息 -->
	<c:choose>
		<c:when test="${type=='hyj'}">
...
<c:choose>
	<c:when test="${type=='hyj'}">
		<div id="querycontent" style="position:absolute;width:25%;height:20%; z-index:2;left:expression((document.body.clientWidth-800)/2);top:expression(this.offsetParent.scrollTop + 20); border:1px solid #6595d6 padding:1px; background:#ffffff; display: none;">
	</c:when>
	<c:otherwise>
...
<c:choose>
	<c:when test="${type=='hyj'}">
		<div style="margin-left: 320px;">
			<input icon="icon-delete" type="button" value="关  闭" onclick="f_close()" />
		</div>
	</c:when>
	<c:otherwise>
...
function f_submit(){
	...
	for(var i = 0;i<ycn.length;i++){
		if(ycn[i].checked == true){
			if(document.getElementById(ycn[i].value)){
...
// 搜索 
function f_search(){
	document.thisForm.action = "${pageContext.request.contextPath}/common/inspection.do?method=company";
	document.thisForm.submit();
}
