package cn.org.gdicpa.web.action.bbwhnj;
...
public class WhnjbAction extends MultiActionController{
	private static String LIST = "/bbwhnj/list.jsp";
	private static String ADDANDEDIT = "/bbwhnj/addAndEdit.jsp";
	private static String SEARCH = "/bbwhnj/search.jsp";
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteWhnjTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbwhnj.do?method=index&typeid="+typeid);
		return null;
	}
