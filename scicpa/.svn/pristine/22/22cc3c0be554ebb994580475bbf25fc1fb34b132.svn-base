...
<form id="thisForm" name="thisForm" method="post" action="">
	<div style="width:100%;">
			...
			<div id="search" style="position:absolute;width:50%;height:170px; z-index:0;left:expression((document.body.clientWidth-575)/2);top:expression(this.offsetParent.scrollTop + 70); border:1px solid #6595d6; padding:1px; background:#ffffff; display: none;">
			    ...
			    <fieldset  style="border: 1px solid lightblue;width:97%;height: 60%;margin-left: 2px;" id="fieldset">
					...
					<table style="margin-left: 1px;width: 80%" align="center">
						...
						<tr >
							<td style="text-align: right;">CPA证书号：</td>
							<td >
		...
		<div style="width:100%;overflow:scroll;height:100%px;"> 
			<mt:DataGridPrintByBean name="certificateList"/>
		</div>
		</div>
	</div>
...
<script>
	...
	function f_clear(){
		...
		// 省注协会员机链接
		if(source=="hyj"){
			document.getElementById("thisForm").action = "${pageContext.request.contextPath}/common/certificateReceipt.do?method=certificateListFromHYJ"; 
		}else{
			document.getElementById("thisForm").action = "${pageContext.request.contextPath}/common/certificateReceipt.do?method=certificateList"; 
		} 
	...
	// 打印
	function printCertificate(id,loginid){
		var url="${pageContext.request.contextPath}/common/certificateReceipt.do?method=checkJHKY";
		var request = "&id="+id+"&micfoLoginid="+loginid;
		var resText = ajaxLoadPageSynch(url,request);
		var url = "";
		if(resText!=null){
			var arr = resText.split(":");
			//var s = encodeURI("","UTF-8");	
			url = "http://ce.esnai.net/c/cpaguangdong/printcert.jsp"+
				"?realname="+arr[0]+"&certno="+arr[1]+"&year="+arr[2]+"&trainingname="+arr[3]+"&studytime="+arr[4]+"";
			url = encodeURI(url,"UTF-8");
			if(arr[5]=='上海会院'){
				window.location = url;
			}else{
				window.showModalDialog("${pageContext.request.contextPath}/common/certificateReceipt.do?method=printCertificate&id="+id+"&micfoLoginid="+loginid,'','dialogWidth=900px;dialogHeight=1000px;');
			}
		}else{
			window.showModalDialog("${pageContext.request.contextPath}/common/certificateReceipt.do?method=printCertificate&id="+id+"&micfoLoginid="+loginid,'','dialogWidth=900px;dialogHeight=1000px;');
		}
		// window.open("${pageContext.request.contextPath}/common/certificateReceipt.do?method=printCertificate&id="+id+"&micfoLoginid="+loginid);
	}
</script>