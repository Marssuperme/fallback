	...
	<div style="width:100%;">
	    ...
	    <div class="gridFrame">
		    ...
		    <div style="width:100%;overflow:auto;height:300px;"> 
		    	<mt:DataGridPrintByBean name="bbwhnj"/>
		    </div>
		</div>
...
<script>
	...
	// 查看是否审核
	function isReviewed(){
		var chooseValue = document.getElementById('chooseValue_bbwhnj').value ;
	...
	// 添加
	function f_add(){
		document.myform.action = "${pageContext.request.contextPath}/common/bbwhnj.do?method=addAndEdit&param=add";
			    ...
			    { 
					...
					if(resText=="true"){
						...
						if(p=="Y"){
							...
							document.myform.action = "${pageContext.request.contextPath}/common/bbwhnj.do?method=addAndEdit&GUID="+chooseValue+"&param=update";
							document.myform.submit();
						}
					}
				 }
		...
		if(chooseValue==null || chooseValue==""){
			...
			if(confirm("确定要删除吗？")){
				...
				if(p=="Y"){
					...
					myform.action = "${pageContext.request.contextPath}/common/bbwhnj.do?method=deleteWhnjTable&GUID="+chooseValue;
					myform.submit();
				}
			}
		}
	...
	function f_viewTD(guid,companyguid){
		document.myform.action = "${pageContext.request.contextPath}/common/bbwhnj.do?method=addAndEdit&GUID="+guid+"&param=search&p=ck";
		document.myform.submit();
	}
			    ...
			    { 
					...
					if(resText=="true"){
						...
						if(p=="Y"){
							...
							document.myform.action = "${pageContext.request.contextPath}/common/bbwhnj.do?method=addAndEdit&GUID="+chooseValue+"&param=search&p=zf";
							document.myform.submit();
						}
					}
				 }
