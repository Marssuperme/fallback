...
<input type="hidden" id="ahattachmentid" name="ahattachmentid" value="${ah.attachmentid }">
<input type="hidden" id="attachmentid" name="attachmentid" value="${attachmentid }">
<input type="hidden" id="hourAttachmentid" name="hourAttachmentid" value="${hourAttachmentid}">
...
<br>
			<input type="button" size="25" value="保存" icon="icon-save" onclick="f_save()">&nbsp;&nbsp;&nbsp;&nbsp;
...
function f_save(){
	...
	if(id != null && id != ""){
		document.myform.action = "${pageContext.request.contextPath}/common/applyHoldSelfHour.do?method=updateApplyTrainHourTable";
	}else{
		document.myform.action = "${pageContext.request.contextPath}/common/applyHoldSelfHour.do?method=addApplyHoldSelfHourTable";
	}
