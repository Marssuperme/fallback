...
<input id="id" name="id" type="hidden" value="${aTrainHourElse.id }">
<input id="loginid" name="loginid" type="hidden" value="${aTrainHourElse.loginid }">
<input id="loginName" name="loginName" type="hidden" value="${aTrainHourElse.loginName }">
<input id="companyChecked1" name="companyChecked1" type="hidden" value="${aTrainHourElse.companyChecked }">
<input id="provinceChecked1" name="provinceChecked1" type="hidden" value="${aTrainHourElse.provinceChecked }">
<input type="hidden" id="attachmentid" name="attachmentid" value="${aTrainHourElse.attachmentid }">
<input type="hidden" id="attachmentid1" name="attachmentid1" value="${attachmentid}">
<input id="add" name="add" type="hidden" value="add"><!-- 判断是否执行新增，还是修改 -->
...
<table style="WIDTH: 100%; border: 0px;" class="data_tb" align="center">
	...
	<tr>
		...
		<td class="data_tb_content" width="30%" height="18" colspan="3"> 
			<input maxlength="50" name="applyHours" style="width: 25%" type="text" id="applyHours" value="${aTrainHourElse.applyHours }" class="required number">
		</td>
	</tr>
	...
	<tr>
	 ...
	 <tr>
		...
		<td class="data_tb_content" width="80%" height="18" colspan="3"> 
			<textarea name="educationNote" id="educationNote" style="width: 90%" rows="8">${aTrainHourElse.educationNote}</textarea> 
		</td>
	</tr>
</table>
<br>
...
var id = "${aTrainHourElse.attachmentid}";
if(id!=null && id!=""){
	try{
		$('#file').uploadFile({
...
function f_save(){
	...
	document.myform.action = "${pageContext.request.contextPath}/common/ApplyTrainHourElse.do?method=save";
}
