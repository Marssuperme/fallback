...
<form action="" method="post" name="myform" id="myform"> 
	...
	<div style="width:100%;">
		   ...
		   <fieldset  style="border: 1px solid lightblue;width:100%;height: 70%;margin-left: 2px;" id="fieldset">
		    	...
		    	<mt:DataGridPrintByBean name="mTrainHourList"/>
...
// 发起申请
function f_add(){
	document.myform.action = "${pageContext.request.contextPath}/common/MicfoApplyTrainHour.do?method=addGo";
...
//修改
function update(){
	var chooseValue = document.getElementById('chooseValue_mTrainHourList').value ;
	
	if(chooseValue==null || chooseValue==""){
		alert("请选择要修改的记录!");
	}else{	
		    document.getElementById("id").value=chooseValue;
			document.myform.action = "${pageContext.request.contextPath}/common/MicfoApplyTrainHour.do?method=selectmTrainHourId";
			document.myform.submit();
		}
	}
...
function f_delete(){
	...
	if(chooseValue==null || chooseValue==""){
		...
		if(confirm("您确定要删除吗？")){
			document.myform.action = "${pageContext.request.contextPath}/common/MicfoApplyTrainHour.do?method=deletemTrainHour&id="+chooseValue;
			document.myform.submit();
		}
	}
}
