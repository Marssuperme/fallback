	    ...
	    <div class="gridFrame" style="height: 80%;">
		    ...
		    <div id="search" style="position:absolute;width:50%;height:15%; z-index:0;left:expression((document.body.clientWidth-400)/2);top:expression(this.offsetParent.scrollTop + 70); border:1px solid #6595d6; padding:1px; background:#ffffff; display: none;">
			    ...
			    <fieldset  style="border: 1px solid lightblue;width:95%;height: 70%;margin-left: 2px;" id="fieldset">
					...
					<table>
			    		<tr>
			    			<td colspan="2">&nbsp;&nbsp;&nbsp;&nbsp;转所时间：&nbsp;&nbsp;&nbsp;&nbsp;<input id="outTime1" name="outTime1" showcalendar="true"></td>
			    		</tr>
			    		<tr>
		    ...
		    <div style="width:100%;overflow:auto;height:85%;"> 
		    	<mt:DataGridPrintByBean name="officeRecord"/>
		    </div>
		</div>
...
<script type="text/javascript">
	// 查看详细
	function f_view(){
		var chooseValue = document.getElementById('chooseValue_officeRecord').value ;
		if(chooseValue==null || chooseValue==""){
			alert("请选择要查看的记录!");
		}else{
			myform.action = "${pageContext.request.contextPath}/common/officeRecord.do?method=view&id="+chooseValue;
			myform.submit();
		}
	}
	...
	function f_clear(){
		...
		goSearch("officeRecord");
	}
	
	// 增加
	function f_add(){
		myform.action = "${pageContext.request.contextPath}/common/officeRecord.do?method=addAndEdit&param=add";
		myform.submit();
	}
	...
	function f_update(){
		...
		if(chooseValue==null || chooseValue==""){
			...
			myform.action = "${pageContext.request.contextPath}/common/officeRecord.do?method=addAndEdit&param=update&id="+chooseValue;
			myform.submit();
		}
	}
	...
	function f_delete(){
		...
		if(chooseValue==null || chooseValue==""){
			...
			if(confirm("确定要删除吗？")){
				myform.action = "${pageContext.request.contextPath}/common/officeRecord.do?method=deleteOfficeRecordTable&param=update&id="+chooseValue;
				myform.submit();
			}
		}
	}
