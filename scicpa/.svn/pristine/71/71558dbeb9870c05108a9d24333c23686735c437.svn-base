				...
				<tbody>
					...
					<tr>
						...
						<td class="data_tb_content" width="30%" height="39"><input
							...
							value="${userMap.borndate }" showcalendar="true" onkeypress="return false;" onpaste="return false;"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="data_tb_content" height="18">
								<select id="isshareholder" name="isshareholder" style="width: 55px;" cannotedit>
									<option value="否" <c:if test="${userMap.isshareholder == '否' || userMap.isshareholder=='' || empty userMap.isshareholder}">selected</c:if> >否</option>
					...
					<tr>
						...
						<td class="data_tb_content" height="18"><input name="filestorage" type="text" id="filestorage"  size="25"  value = "${userMap.filestorage }"></td>
					...
					<tr>
						...
						<td class="data_tb_content" height="18" colspan="3">
							<input name="certificate" type="text" id="certificate"  size="25"  value = "${userMap.certificate }" cannotedit>
						</td>
					</tr>
					<tr>
						<td class="data_tb_alignright" style="text-align: left;"
...
function upLoadSumbit() {
	...
	if(fileName.indexOf(".") > -1) { 
		...
		if(photo == "jpg" || photo == "JPG"){
			...
			var loginid = "${loginid}";
			var tabname = "${tabname}";
			myForm.action = "${pageContext.request.contextPath}/micfo/micfo.do?method=uploadPhoto&loginid="+loginid+"&tabname="+tabname ;
			hiddenProDiv();
			return true;
		}else{
			alert("只允许上传jpg格式照片!");
...
function isAudit(){
	var loginid = "${loginid}";
	var oBao = new ActiveXObject("Microsoft.XMLHTTP");
	var url="${pageContext.request.contextPath}/micfo/micfo.do?method=isAudit&loginid="+loginid;
	oBao.open("POST",url,false);
	oBao.send();
