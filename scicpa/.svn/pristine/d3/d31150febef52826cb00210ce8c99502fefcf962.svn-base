...
<div class=tabcontent id="nre"
   ...
   <table width="100%" border="0" cellpadding="0" cellspacing="0" class="data_tb" align="center">
 	  <tr>
	    <td width="123" height="53" class="data_tb_alignright">代理项目<font color="#FF0000">&nbsp;[*]</font></td>
	    <td width="577" height="53" colspan="3" align="left" >
	    	<textarea title="代理项目" id="dlxm" name="dlxm" style="width: 450px;height: 50px;" class="required">${st.dlxm }</textarea>
	    </td>
	  </tr>
   </table>
...
function saveCompanyJudge(){
	var swdlbguid = document.getElementById("swdlbguid").value;
	if(swdlbguid=="" || swdlbguid==null){
		document.thisForm.action="${pageContext.request.contextPath}/common/bbswdlb.do?method=addSwdlbTable";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}else{
		document.thisForm.action="${pageContext.request.contextPath}/common/bbswdlb.do?method=updateSwdlbTableById";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}
...
// 返回
function goback(){	
	document.thisForm.action="${pageContext.request.contextPath}/common/bbswdlb.do?method=index&typeid=swdl001";
	thisForm.submit();
}
