...
<div id="tzf" style="position:absolute;width:80%;height:20%; z-index:2;left:expression((document.body.clientWidth-550)/2);top:expression(this.offsetParent.scrollTop + 130); border:1px solid #6595d6; padding:10px; background:#ffffff;text-align:center; display: none;">
   <table width="90%">
   	<tr>
    	...
    	<textarea title="作废原因" class="data_tb_content required" id="bbreason" name="bbreason" style="width: 80%;height: 100px;vertical-align: middle;"> 
    	</textarea>
    	</td>
    </tr>
    <tr>
...
function f_sure(){
	...
	if(bbreason=="" || bbreason==null){
		...
		if(confirm("您确定要作废吗?")){
			...
			var url="${pageContext.request.contextPath}/common/content.do?method=invalid&ctguid="+ctguid+"&bbreason="+bbreason+"&reportCount="+reportCount;
