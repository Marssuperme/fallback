package cn.org.gdicpa.web.action.bbsj;
...
public class SjAction extends MultiActionController{
	private static String LIST = "/bbsjb/list.jsp";
	private static String ADDANDEDIT = "/bbsjb/addAndEdit.jsp";
	private static String SEARCH = "/bbsjb/search.jsp";
	...
	public ModelAndView index(HttpServletRequest request,HttpServletResponse response){
		System.out.println(this.getClass()+"  >>>>>>>>>>>> 111111122222     index(HttpServletRequest request,HttpServletResponse response) ...............LIST="+LIST);
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteSjTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbsjb.do?method=index&typeid="+typeid);
		return null;
	}
	...
	public ModelAndView addAndEdit(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			if(param.equals("search")){
				...
				// 审计
				SjService ss = new SjService(conn);
				String p = request.getParameter("p");   //  是查看操作还是作废操作的判断依据
				String GUID = request.getParameter("GUID");
				// 代报备内容
