			    ...
			    <fieldset  style="border: 1px solid lightblue;width:97%;height: 60%;margin-left: 2px;" id="fieldset">
					...
					<table style="margin-left: 1px;width: 80%" align="center">
						...
						<tr >
							...
							<td >
								<input id="cpano" name="cpano" size="35" style="height: 0px;" value="${cpano }">
							</td>
						</tr>	
					</table>
					<br>
...
<script>
	...
	// 打印
	function printPeriod(PeriodResult,loginid,cpano,loginname,curryear){
		//loginname=encodeURI(loginname,'GBK');
		//alert(loginname);
		window.showModalDialog("${pageContext.request.contextPath}/common/certificateReceipt.do?method=printPeriod&PeriodResult="+PeriodResult+"&micfoLoginid="+loginid+"&cpano="+cpano+"&loginname="+loginname+"&curryear="+curryear,'','dialogWidth=900px;dialogHeight=1000px;');
