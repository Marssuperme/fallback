...
var TEXTVALUE2 = null; //保存下接列表的第2列的值
...
var TEMP_SYSTEM_WEB_ROOT = "/web/";

if(CONTEXT_PATH && CONTEXT_PATH!="") {
	TEMP_SYSTEM_WEB_ROOT = CONTEXT_PATH;
}

var DEFAULT_REFRESHURL= TEMP_SYSTEM_WEB_ROOT + "/common/hint.do";
var NOSHOW_HINT;
var _myHistory;
...
function HideSelect(){
  ...
  var AUTO_HINT_LIST = document.getElementById("AUTO_HINT_LIST") ;
...
function ToggleTags(obj){
    ...
    AUTO_HINT_IFRAME.style.height = obj.offsetHeight;   
  ...
  try{
	...
	*/
	...
	if (suburl.indexOf("请先完成")>=0){
		...
		createXmlHttp();
		suburl = encodeURI(suburl,"UTF-8");//下拉加上编码
  ...
  try {
		...
		if (CURRENT_SELECT_OBJ.multiselect) {
			...
			for (var i=2; i<lineArray.length-1; i++) {
				...
				//如果只有1行并且等于当前选择值，就不显示，直接隐藏
				if ( txtArray[0]==theSelectCode){
					setSelectionValueAndName(txtArray[0],txtArray[1]);
				}
			}
		}
...
function getAdviceObj(obj){
	...
	try{
		...
		//查找对应的值
		adviceobj=document.getElementById(getAdviceName(obj));
...
function setSelectionValueAndName(pk1,pk1_name){
  ...
  try{
    ...
    if (!CURRENT_SELECT_OBJ) return;
  	...
  	if (CURRENT_SELECT_OBJ.hideresult=="true"){
  	}else
	...
	if (CURRENT_SELECT_OBJ.value!=pk1){
		...
		TEXTVALUE2 = pk1_name;//把第2列值保存在TEXTVALUE2里
	...
	for ( i = 0; i < oInputs.length; i++ ) {
		...
		if (oInputs(i).autoid ){
			...
			*/
			...
			//是否检查下拉内容是否存在
			if (oInputs(i).valuemustexist=="true")
				oInputs(i).onblur=function(){
					if(this.value!=""){
						var url = DEFAULT_REFRESHURL + "?method=checkValeExists&autoid="+this.autoid+"&refer="+this.refer+"&value="+this.value;
						var ajaxObj = new ActiveXObject("Microsoft.XMLHTTP");
						//oBao.asynchronous=false;
						ajaxObj.open("POST",encodeURI(url,"UTF-8"),false);
						ajaxObj.send();
						var resText = unescape(ajaxObj.responseText);
						if(resText=="NO"){
							this.value="";
							return;
						}else if(resText.indexOf("ERROR|")>-1){
							alert(resText);
							this.value="";
							return;
						}else if(resText.indexOf("OK|")==-1){
							alert("系统异常，请联系管理员！");
							this.value="";
							return;
						}
