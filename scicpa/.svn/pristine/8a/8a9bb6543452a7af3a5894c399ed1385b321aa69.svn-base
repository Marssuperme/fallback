package cn.org.gdicpa.web.action.bbbbqtb;
...
public class BbqtbAction extends MultiActionController{
	private static String LIST = "/bbbbqtb/list.jsp";
	private static String ADDANDEDIT = "/bbbbqtb/addAndEdit.jsp";
	private static String PRINT = "/bbbbqtb/print.jsp";
	private static String PRINTLIST = "/bbbbqtb/printList.jsp";
	private static String SEARCH = "/bbbbqtb/search.jsp";
...
//			userMap.put("area", clt.getArea());//事务所归属地
	 ...
	 * @throws IOException 
	 */
	public ModelAndView deleteBbqtbTable(HttpServletRequest request,HttpServletResponse response) throws IOException{
		Connection conn = null;
		String typeid = request.getParameter("typeid");
		try {
			conn = new DBConnect().getConnect();
		...
		response.sendRedirect("bbbbqtb.do?method=index&typeid="+typeid);
		return null;
	}
...
//				}
	...
	public ModelAndView printList(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			String sql = "select * from bb_view ";
			...
			pp.addColumn("委托单位名称", "WTDWMC");
			//pp.addColumn("业务类型", "typename");
			pp.addColumn("被审（验）单位", "bsdwmc");
			
			pp.addColumn("业务约定书号码", "ywyds");
			pp.addColumn("报告年度", "BGND");
			pp.addColumn("报告文号", "BGWH");
			
			
			pp.addColumn("报备编号", "bbbh");
			pp.addColumn("报备状态 ", "bbstate");
			pp.addColumn("是否审核", "isreviewed");
			pp.addColumn("注师1", "QMZS1");
			pp.addColumn("注师2", "QMZS2");
			pp.addColumn("注师3", "QMZS3");
