...
<div id="divBlock" style="position:absolute;width:27%;;height:100%; z-index:0; padding:10px; background:#ffffff;filter:alpha(opacity=50); text-align:center; display:none;"></div>
...
<form action="" method="post" name="myform" id="myform"> 
	...
	<div style="width:100%;">
		   ...
		   <fieldset  style="border: 1px solid lightblue;width:100%;height:70%;margin-left: 2px;" id="fieldset">
		    	...
		    	<mt:DataGridPrintByBean name="aplyTinList"/>		    
		</div>
</form>
...
<script type="text/javascript" defer="defer">
	// 发起申请
	function f_add(){
		document.myform.action = "${pageContext.request.contextPath}/common/ApplyTrainHourElse.do?method=addGo";
	...
	//修改
	function update(){
		var chooseValue = document.getElementById('chooseValue_aplyTinList').value ;
		
		if(chooseValue==null || chooseValue==""){
			alert("请选择要修改的记录!");
		}else{	
		    document.getElementById("id").value=chooseValue;
			document.myform.action = "${pageContext.request.contextPath}/common/ApplyTrainHourElse.do?method=selectCteExtionId";
			document.myform.submit();
		}
	}
	...
	function f_delete(){
		...
		if(chooseValue==null || chooseValue==""){
			...
			if(confirm("您确定要删除吗？")){
				document.myform.action = "${pageContext.request.contextPath}/common/ApplyTrainHourElse.do?method=deleteFcotetExtion&id="+chooseValue;
				document.myform.submit();
			}
		}
	}
	...
	function goView(id){
		...
		document.myform.action = "${pageContext.request.contextPath}/common/ApplyTrainHourElse.do?method=view";
		document.myform.submit();
	}
</script>
