...
<input type="hidden" id="kjzsbguid" name="kjzsbguid" value="${kt.guid}">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="data_tb" align="center">
 	  <tr>
	    <td width="123" height="53" class="data_tb_alignright">咨询项目<font color="#FF0000">&nbsp;[*]</font></td>
	    <td width="577" height="53" colspan="3" align="left" >
	    	<textarea title="咨询项目" id="zsxm" name="zsxm" style="width: 450px;height: 50px;" class="required">${kt.zsxm }</textarea>
	    </td>
	  </tr>
</table>
...
function saveCompanyJudge(){
	var kjzsbguid = document.getElementById("kjzsbguid").value;
	if(kjzsbguid=="" || kjzsbguid==null){
		document.thisForm.action="${pageContext.request.contextPath}/common/bbkjzsb.do?method=addKjzsbTable";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}else{
		document.thisForm.action="${pageContext.request.contextPath}/common/bbkjzsb.do?method=updateKjzsbTableById";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}
...
// 返回
function goback(){	
	document.thisForm.action="${pageContext.request.contextPath}/common/bbkjzsb.do?method=index&typeid=kjzs001";
	thisForm.submit();
}
</script>
