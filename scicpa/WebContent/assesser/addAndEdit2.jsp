...
<H3 class=tabs id=searchtabs><A class="tab curtab" id=dgxxtab
	href="javascript:setTab('search','dgxx')">评估信息</A> <A class="tab"
	...
	<tr align="center">
		...
		<td width="33%" align="left">
			<div id="fhc1"><input type="button" name="next"
				value="返  回" class="flyBT" icon="icon-back" onclick="window.history.back()">
			</div>
		</td>
	</tr>
</table>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td colspan="2" align="left">
			<input class="data_tb_content"
			name="loginid2" type="text" id="loginid2" size="20"
			value="${userMap.loginid }" disabled="disabled">
			<input name="loginid" type="hidden" id="loginid"  
			value="${userMap.loginid }">
		</td>
	</tr>
	<tr>
	...
	<tr align="center">
		...
		<td width="20%">
			<div style="display: none" id="bcc">
				<input type="button" style="display: none;" name="bcb" id="bcb" value="保  存" icon="icon-save" onclick="return saveAssesser();">
			</div>
		</td>
		<td width="33%" align="left">
			...
			<table style="WIDTH: 100%" class="data_tb" align="center">
				<tbody>
					...
					<tr>
						...
						<td class="data_tb_alignright" width="50%" style="text-align: center" colspan="2" rowspan="4"
							height="111"><img id="bill" height="120" width="100" src="${pageContext.request.contextPath}${userPhotoSrc }"><br>
								...
								<span id="logospan" style="display: none;">
									...
									<a style="color: red">*注意：请上传白底免冠正面头像,格式jpg,<br>&nbsp;&nbsp;像素：宽度*高度(295*413),大小20k以下!</a>
									<!-- 
					...
					<tr>
						...
						<td class="data_tb_content" width="30%" height="21">
							<input name="sex" type="text" id="sex" size="25" value="${userMap.sex }" myClass="required" title="请输入性别"
					...
					<tr>
						<td class="data_tb_alignright" width="20%" height="18">专业</td>
					...
					<tr>
						<td class="data_tb_alignright" style="text-align: left;"
							height="18" colspan="4"><b>评估信息</b></td>
					</tr>
					<tr>
						<td class="data_tb_alignright" height="18">评估所代码</td>
						<td class="data_tb_content" height="18" colspan="3">
							<input cannotedit name="officecode" type="text" id="officecode"  size="25"  value = "${userMap.officecode }">
						</td>
					</tr>
					<tr>
						<td class="data_tb_alignright" height="18">评估所全称</td>
						<td class="data_tb_content" height="18" colspan="3">
					...
					<tr>
						...
						<td class="data_tb_content" height="18"><input name=assessid type="text" id="assessid"  size="25"  value = "${userMap.assessid }" cannotedit></td>
					...
					<tr>
						...
						<td class="data_tb_alignright" height="18">职称类型</td>
						<td class="data_tb_content" height="18"><input name="rank" type="text" id="rank"  size="25" maxlength="100" value = "${userMap.rank }"></td>
					</tr>
					<tr>
						<td class="data_tb_alignright" height="18">考试档案号</td>
						<td class="data_tb_content" height="18">
							<input name="examno" type="text" id="examno" maxlength="20" size="25"  value = "${userMap.examno }">
						</td>
						<td class="data_tb_alignright" height="18">全科通过日期</td>
						<td class="data_tb_content" height="18">
							<input name="passdate" type="text" id="passdate"  size="25" maxlength="100" value = "${userMap.passdate }" showcalendar="true">
						</td>
					</tr>
					<tr>
						<td class="data_tb_alignright" height="18">人员批准文号</td>
						<td class="data_tb_content" height="18">
							<input name="assesspermitno" type="text" id="assesspermitno"  size="25"  value = "${userMap.assesspermitno }" cannotedit>
						</td>
						<td class="data_tb_alignright" height="18">人员批准时间</td>
						<td class="data_tb_content" height="18">
							<input name="assessapprove" type="text" id="assessapprove"  size="25"  value = "${userMap.assessapprove }" showcalendar="true">
						</td>
					</tr>
					<tr>
						<td class="data_tb_alignright" height="18">档案存放单位</td>
						<td class="data_tb_content" height="18" colspan="3">
							<input name="filestorage" type="text" id="filestorage"  size="25"  value = "${userMap.filestorage }" cannotedit>
						</td>
					</tr>
				</tbody>
			</table>
...
function upLoadSumbit() {
	...
	if(fileName.indexOf(".") > -1) { 
		...
		if(photo == "jpg" || photo == "JPG"){
			...
			var loginid = "${loginid}";
			var tabname = "${tabname}";
			myForm.action = "${pageContext.request.contextPath}/assesser/assesser.do?method=uploadPhoto&loginid="+loginid+"&tabname="+tabname ;
...
function changePhoto(srcName) {
	...
	if(attachments.value != "") {
		document.getElementById("bill").src = "${pageContext.request.contextPath}/common/attachFile/K_Assesser/"+srcName;
...
function deletePhoto() {
	...
	if(userphotoname.value == ""){
		...
		var url="${pageContext.request.contextPath}/assesser/assesser.do?method=deletePhoto&userphotoname="+userphotoname.value+"&loginid=${userMap.loginid}&ctypetabname=${userMap.ctypetabname }" ;
...
function updateStyle(){ 
	...
	document.getElementById("assessapprove").readOnly=true;
 ...
 // 返回处理 
function goBack(){
	document.thisForm.action = "${pageContext.request.contextPath}/assesser/assesser.do?method=goAddFrame&param=mainFrame";
...
function updatePwds(){
	...
	if(updatePwd()){
		var oBao = new ActiveXObject("Microsoft.XMLHTTP");
		var url="${pageContext.request.contextPath}/assesser/assesser.do?method=updatePwds&ctypetabname="+ctypetabname+"&pwd_old="+pwd_old+"&pwd_one="+pwd_one ;
		oBao.open("POST",url,false);
		oBao.send();
...
if( sender.files &&sender.files[0] ){
  ...
  objPreview.style.display ='none';
	...
	document.getElementById("myForm").action = "${pageContext.request.contextPath}/assesser/assesser.do?method=scanPhoto";
...
function isAudit(){
	var oBao = new ActiveXObject("Microsoft.XMLHTTP");
	var url="${pageContext.request.contextPath}/assesser/assesser.do?method=isAudit";
	oBao.open("POST",url,false);
	oBao.send();
