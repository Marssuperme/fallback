...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="data_tb" align="center">
 	  <tr>
	    <td width="123" height="53" class="data_tb_alignright">评价结果<font color="#FF0000">&nbsp;[*]</font></td>
	    <td width="577" height="53" colspan="3" align="left" >
	    	<textarea title="评价结果" id="pjjs" name="pjjs" style="width: 450px;height: 50px;" class="required">${kt.pjjs }</textarea>
	    </td>
	  </tr>
</table>
...
function saveCompanyJudge(){
	var kjdshbguid = document.getElementById("kjdshbguid").value;
	if(kjdshbguid=="" || kjdshbguid==null){
		document.thisForm.action="${pageContext.request.contextPath}/common/bbkjdshb.do?method=addKjdshbTable";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}else{
		document.thisForm.action="${pageContext.request.contextPath}/common/bbkjdshb.do?method=updateKjdshbTableById";
		if (thisForm.fireEvent('onsubmit')){
			document.thisForm.submit();
		}
	}
...
// 返回
function goback(){	
	document.thisForm.action="${pageContext.request.contextPath}/common/bbkjdshb.do?method=index&typeid=kjdsh001";
	thisForm.submit();
}
