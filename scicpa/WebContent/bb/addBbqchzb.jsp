...
<input type="hidden" id="qchzbguid" name="qchzbguid" value="${qt.guid}">
<input type="hidden" id="bguid" name="bguid" value="${qt.guid}">
<input type="hidden" id="addJsp" name="addJsp" value="${pageContext.request.contextPath}/common/bb.do?method=addQchzbTable">
<input type="hidden" id="updateJsp" name="updateJsp" value="${pageContext.request.contextPath}/common/bb.do?method=updateQchzbTableById">

<table border="1" cellpadding="0" cellspacing="0" style="padding-left: 2px" width="100%">
  <tr>
    <td width="15%" height="16" class="data_tb_alignright">资产总额<font color="#FF0000">&nbsp;[*]</font></td>
    <td width="35%" height="16"><input maxlength="20"  id="zcze" value="${qt.zcze}" name="zcze" size="20" onkeyup="f_money(this)" class="required number">万元
	<font color="red">(保留小数点后四位以上，不作四舍五入)</font></td>
    <td width="15%" height="16" class="data_tb_alignright">营业收入<font color="#FF0000">&nbsp;[*]</font></td>
    <td width="35%" height="16"><input maxlength="20" id="xssr" value="${qt.xssr}" name="xssr" size="20" onkeyup="f_money(this)" class="required number">万元
  ...
  <tbody id="tb_tzh_money">
	  <tr>
	    ...
	    <td width="35%" height="23"><input maxlength="20" id="zczetzh" value="${qt.zczetzh }" name="zczetzh" size="20" class="number" onkeyup="f_money(this);" >万元
		<font color="red">(保留小数点后四位以上，不作四舍五入)</font></td>
	   	<td width="15%" height="23" class="data_tb_alignright">营业收入(调整后)<font color="#FF0000">&nbsp;[*]</font></td>
	    <td width="35%" height="23"><input maxlength="20" id="xssrtzh" value="${qt.xssrtzh }" name="xssrtzh" size="20" class="number" onkeyup="f_money(this);" >万元
		<font color="red">(保留小数点后四位以上，不作四舍五入)</font></td>
	  </tr>
  </tbody>
 
  <tr>
   <td width="15%" height="16" class="data_tb_alignright">净增值额</td>
   <td width="35%" height="16" ><input maxlength="20" id="jzze" value="${qt.jzze}" name="jzze" size="20" onkeyup="f_money(this)" class="number" >万元</td>
   <td width="15%" height="16" class="data_tb_alignright">负债总额</td>
   <td width="35%" height="16"><input maxlength="20" id="fzze" value="${qt.fzze}" name="fzze" size="20" onkeyup="f_money(this)" class="number">万元</td>
  </tr>
  ...
  <tr>
   <td width="20%" height="16" class="data_tb_alignright">从业人员<font color="#FF0000"></font></td>
   <td height="16">
   		<input maxlength="20" id="cyry" value="${qt.cyry }" name="cyry" title="从业人员" size="20" class="number" onkeyup="f_number(this);f_checkZero(this)" >人
   		<font color="red">(被审验单位期末员工数)</font>
   </td>
   <td width="15%" height="16" class="data_tb_alignright">截止日期<font color="#FF0000">&nbsp;[*]</font></td>
   <td width="35%" height="16">
   	<input maxlength="50" id="jzrq" value="${qt.jzrq }" size=20 name="jzrq" showcalendar="true" class="required" onkeypress="return false;" onpaste="return false;">
   </td>
  </tr>
</table>
...
function f_checkMoney(isCount,wjjsfbz){
	...
	if((xssr*1+zcze*1)<=0){
		...
		if(bbState!="审核通过" || isyqyyw=="1"){
			...
			if(wjjsfbz=="wjjsfbz"){
				var moneyArray = f_checkMoneyStandard(0,"qchz001");
		...
		// 物价局收费标准：直接提示标准价
		if(wjjsfbz=="wjjsfbz"){
			var moneyArray = f_checkMoneyStandard(ysywfCountTemp,"qchz001");
			var standerMoney = moneyArray[1]*1;
			return "发改委、财政厅收费标准价为："+standerMoney.toFixed(2)+" 元!";
		}else{
			return f_returnInfo(ysywfCountTemp,"qchz001",isCount);
		}
	}
}
