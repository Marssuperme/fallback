...
<H3 class=tabs id=searchtabs>
	...
	<div id="fsave" style="margin-left: 58%;margin-top: 1%">
			...
			<span id="ccSpan" style="display: none;">
				<input type="button" icon="icon-success" name="wcBtn" id="wcBtn" value="初次报备" onclick="saveCcBbAddWait();">
			</span>
			<span id="wcSpan">
	...
	<tr>
	 ...
	 <tr>
		...
		<td class="data_tb_content" height="18"> 
			<input maxlength="100" name="post" myClass="number" class="required" type="text" id="post" style="width:205px;" value = "${clt.post }">
		</td>
	</tr>
...
<table cellSpacing="0" cellPadding="5" class="data_tb" width="100%" >
  ...
  <tr>
    ...
    <td width="20%" height="46" class="data_tb_alignright">被审(验)单位注册资本/开办资金<font color="#FF0000" >&nbsp;[*]（保留小数点后四位以上，不作四舍五入）</font></td>
...
//初次报备时加了等待进度
function saveCcBbAddWait(){
	divBlock.style.display = "";	
	setTimeout("if(saveCcBb()==undefined){divBlock.style.display = 'none';}",1000);
}

//初次报备必填字段
var cCBcArr = ['bgwh','wtdwmc','bsdwmc','bgrq','ywarea','qmzs1','qmzs2','Faxes','address','Email'];
var dsArr = ['报告文号','委托单位名称','被审(验)单位名称','报告日期','被审(验)单位注册地址','签名注师一','签名注师二','传    真','地    址','E-Mail'];
//初次报备
function saveCcBb(){
	for(var i=0;i<cCBcArr.length;i++){
		if($("#"+cCBcArr[i]).val()==""){
			alert(dsArr[i]+"不能为空！");
			setTab('search','nry') ;
			document.getElementById(cCBcArr[i]).select();
			return;
		}
	}	
	...
	document.getElementById("tempORFinish").value="初次报备"; 
...
//收费标准(计算的地方)------------------------------------------------------------------------------
	...
	if(yzbguid=="" || yzbguid == null){
		...
		if (thisForm.fireEvent('onsubmit')){
			if(confirm("点击报备完成后，只有客户信息的注册地址、负责人姓名、联系人姓名、联系人电话在一个月内可以修改，还有收费信息的已收业务费、收费日期、开具发票收款号码的第2个框、收费说明可以修改，其他的修改必须向省注协申请，请确认是否报备完成？")){
				// 罩层 
				document.getElementById("divBlock2").style.display = "";
				thisForm.submit();
			}
...
function f_convertState(state){
	var chState = ["暂停","暂存","申请审核","初审通过","审核未通过","审核通过","报备完成","待审申请审核","待审报备完成","初次报备"];
	var enState = ["zanting","zancun","sqsh","cstg","shwtg","shtg","wanch","dssqsh","dsbbwc","ccbb"];
...
<script>
	...
	//是否可以二次报备
	function isTowBb(o){
		var securities = "${userSession.userMap.securities}";
		if(o.value=="上市企业" && securities=="是"){
			$("#ccSpan").css("display","");
		}else{
	...
	isTowBb(document.getElementById("sfssqy"));
</script>
</html>