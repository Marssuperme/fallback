...
<c:forEach var="print" items="${listPrint }" varStatus="status" >


<table align="center" style="margin-top:20px;" width="100%" border="0">
	<tr align="right"><td>
		<table align="center" cellpadding="0" cellspacing="4"  width="100%" border="0">
			...
			<tr>
				...
				<td width="100">
				 ...
				 <td rowspan="10" width="45%" style="vertical-align: top;">
					<c:choose>
						<c:when test="${print.bbstate=='报备完成' || print.bbstate=='初次报备'}">
							<img src="${pageContext.request.contextPath}/common/content.do?method=getQrcode&printType=printBatch&guid=${print.guid}&v=<%=System.currentTimeMillis() %>" height="150" width="150"/>
							<br>
	...
	<tr align="center">
		<td id="contents">
			...
			<div style="font-weight: bold;font-size:25px;text-align: center;"> 
				<c:if test="${print.typename=='报备其他'}">
				 			${print.bgnd}${print.bustype}报告 
				</c:if>
				<c:if test="${print.typename=='审计'}">
					<c:choose>
						<c:when test="${print.sjlx=='其他专项审计' || print.sjlx=='经济责任审计' || print.sjlx=='改制审计' || print.sjlx=='财务收支审计业务' || print.sjlx=='涉及经济案件审计' || print.sjlx=='基本建设工程决算审核' || print.sjlx=='其他专项审计'}">
					 			<c:choose>
									<c:when test="${!empty print.zxsjmc || print.zxsjmc!=''}">
							 			${print.bgnd}${print.zxsjmc}专项审计报告 
						 			</c:when>
						 			<c:otherwise>
						 				${print.bgnd}${print.sjlx}报告 
						 			</c:otherwise>
						 		</c:choose>
						</c:when>
						<c:otherwise>
				...
				<c:if test="${print.typename!='报备其他' && print.typename!='审计'}">
				 			${print.bgnd}${print.typename}报告 
				</c:if>
			</div>
			<br>
 		</td>
 	...
 	<tr align="center"><td>
	 	<table align="left" cellpadding="0" cellspacing="7" width="100%" border="0">
			<tr>
				<td align="right" nowrap="nowrap" width="35%">
						事务所名称：
				</td>
	...
	<tr align="center">
		<td>
			<table align="center" cellpadding="0" cellspacing="5" width="30%" border="0" >
				<tr>
